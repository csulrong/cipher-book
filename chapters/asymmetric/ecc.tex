...
\startsection[title={椭圆曲线加密算法},reference=sec:ecc]
\index{sec:ecc}
...
\startplacefigure[title={椭圆曲线形状随$a$、$b$的变化},reference=fig:ecc-curves]
...
首先，我们先去掉一些特殊情况，只会考虑两个非无穷远点$P (x_P, y_P)$、$Q (x_Q, y_Q)$。我们针对$P$和$Q$是否对称这两种
情况分别考虑。
...
这里的$0$仍然是无限远的点，$a$和$b$是$\mathbb{F}_p$上的两个整数。\in{图}[fig:ecc-fp]给出了椭圆曲线
$y^2=x^3-3x+10$在有限域$\mathbb{F}_{19}$、$\mathbb{F}_{19}$和$\mathbb{F}_{19}$上的图形。从几何的角度，
...
在实数域，三点共线意味着能够找到一条直线将三个点连在一起。当然，在$\mathbb{F}_p$域中，直线与实数域中的是有所不同的。
...
连接点$P=(13,124)$和$Q=(80,50)$的方程$y \equiv -3x+36 \pmod{127}$在图中多次重复，这是因为$\pmod{127}$的原因。
方程将$P$、$Q$连接上之后，和域中的$R=(43,34)$“相交”，$R$的逆元$-R=(43,-34 \pmod{127})=(43,93)$，也就有
$(13,124) + (80,50) = (43,93)$。
...
这意味着：如果我们将$P$的倍乘进行相加，我们获得的仍然是$P$的倍数。这个性质非常重要，它足够证明：{\bf $nP$的集合是
...
首先，我们要定义一下在一个群有多少个点就叫做这个群的“阶”（order）。群举椭圆曲线在有限域$\mathbb{F}_p$中所有可能的值
的时间复杂度为$O(p)$。当$p$很大的时候，这算下来就很慢很慢，有些不太可行。好在有一个更快的算法来计算阶---Schoof算法。
这里，我们不对Schoof算法的细节进行展开，只需要知道它的复杂度是多项式时间。

在循环的子群里我们可以下一个新的、与前面的定义相等的定义，由$P$生成子群的阶是最小的正整数$n$，$n$满足的条件是$nP=0$。
前面的例子中，$5P=0$，那由$P=(3,6)$生成的子群的阶就等于5。由$P$生成子群的阶不能使用Schoof的算法，因为这个算法只能
在整个椭圆曲线上生效，在子群上无效。
