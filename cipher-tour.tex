...
这种想法是绝对错误的。任何时候，我们都不要尝试自己去发明新的加密算法。正确的做法应该是，使用现在已经公开的经过长期测试
的可靠的加密算法。原因是，这些公开的加密算法经过了密码工作研究者的长期研究和测试，以及在工程实践中经受过长年累月的持久
考验，它们已经被证实过具有非常强的抗破解能力，在当今计算机系统的算力下，很难在现实的时间内被破解。那些你以为自己设计好
的加密算法，不可能像这些公开的加密算法那样得到密码学家们的研究和测试，很可能被攻击者找到漏洞并破解。试图对密码算法本身
进行保密的做法是根本行不通的，存在巨大的安全隐患。
