...
分组长度通常是64比特、128比特、192比特和256比特等。分组密码对每个分组逐个依次进行加密操作。分组长短影响密码
强度，分组长度不能太短也不能太长。既要便于操作与运算又要保证密码的安全性。本章将详细介绍几种分组密码算法：
DES、3DES、AES和Blowfish。分组密码算法一次只能处理固定长度的分组，但是我们需要加密的明文长度可能会超过分组
密码的分组长度，这时就需要对分组密码算法进行迭代，以便将一段很长的明文全部加密，迭代的方法就称为分组密码的分组
模式 (mode)。我们将把分组密码的模式留在下一章进行讲解。
